// public/js/quiz-logic-xemay.js - Ôn tập xe máy (250 câu) với chapter filtering và câu liệt
document.addEventListener('DOMContentLoaded', async function () {
  ...
  let fullGrid = [];      // [1,2,3,...] -> stt_250 only
  ...
  // Cập nhật số lượng câu trong mỗi tab
  function updateTabCounts() {
    if (!chapterTabs) return;

    // Đếm số câu mỗi chương
    const counts = {};
    counts[0] = fullGridData.length; // Tất cả
    counts['liet'] = fullGridData.filter(item => item.cauliet === 1).length;

    fullGridData.forEach(item => {
      const ch = item.chuong || 1;
      counts[ch] = (counts[ch] || 0) + 1;
    });

    chapterTabs.querySelectorAll('.chapter-tab').forEach(tab => {
      const chId = tab.dataset.chapter;
      const countEl = tab.querySelector('.tab-count');
      if (countEl) {
        if (chId === 'liet') {
          countEl.textContent = counts['liet'] || 0;
        } else {
          countEl.textContent = counts[parseInt(chId)] || 0;
        }
      }
    });
  }
  ...
  function filterByChapter(chapterKey) {
    ...
    if (chapterKey === 'liet') {
      ...
      // Lọc theo chương
      grid = fullGridData
        .filter(item => item.chuong === chapterKey)
        .map(item => item.stt);
